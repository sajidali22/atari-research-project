...
class EmaVectorQuantizer(nn.Module):
    """
    Vector Quantizer with Exponential Moving Average (EMA) and Dead Code Revival.
    Ensures all embeddings actively participate in feature extraction.
    """
    ...
    def forward(self, inputs):
        ...
        # One-hot encodings for code usage
        encodings = torch.zeros(encoding_indices.shape[0], self.num_embeddings, device=inputs.device)
        encodings.scatter_(1, encoding_indices, 1)

        # 3. Safe Un-flattening: Retrieve codes -> reshape to [B, H, W, C] -> permute to [B, C, H, W]

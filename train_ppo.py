...
# The modern 2026 Atari registration block
...
from PPO.factory import PPOAgent
...
def main():
    ...
    run_name = f"PPO_{config['extractor']['arch_type']}_JOINT_{config['env']['id']}"
    ...
    video_folder = f"logs/videos/{run_name}"
    os.makedirs(video_folder, exist_ok=True)
    # Records a 500-frame video every 100,000 global steps
    ...
    for update in range(1, num_updates + 1):
        ...
        log_dict = {
            ...
            "charts/loss": loss.item(), 
            "global_step": global_step,
            "visuals/action_distribution": wandb.Histogram(b_actions.cpu().numpy())
        }
        
        # Log the raw 84x84 frame the agent sees every 50 updates
        if update % 50 == 0:
            # Grab env 0, most recent frame from the stack
            current_frame = b_obs[0, :, :, -1].cpu().numpy() 
            log_dict["visuals/agent_view"] = wandb.Image(current_frame, caption=f"Step: {global_step}")
            
        wandb.log(log_dict)
        if update % 500 == 0:
            ckpt_folder = f"logs/checkpoints/{run_name}"
            os.makedirs(ckpt_folder, exist_ok=True)
            torch.save(agent.state_dict(), f"{ckpt_folder}/update_{update}.pth")
    ...
    ckpt_folder = f"logs/checkpoints/{run_name}"
    os.makedirs(ckpt_folder, exist_ok=True)
    final_path = f"{ckpt_folder}/final_model.pth"
    torch.save(agent.state_dict(), final_path)
    print(f"Model successfully saved to {final_path}")
    
    # Push all recorded videos AND model weights to W&B
    print("Uploading artifacts to Weights & Biases...")
    wandb.save(f"{video_folder}/*.mp4", base_path=video_folder)
    wandb.save(f"{ckpt_folder}/*.pth", base_path=ckpt_folder)
    wandb.finish()
